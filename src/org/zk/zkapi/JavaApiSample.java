...
import org.zk.master.GetChildWatcher;
import org.zk.waitq.ReadWatcher;
import org.zk.waitq.WriteWatcher;

public class JavaApiSample implements Watcher {

	public static final int SESSION_TIMEOUT = 10000;
	public static final String CONNECTION_STRING = "hadoop1:2181,hadoop2:2181,hadoop3:2181";
	public static final String ZK_PATH = "/test";
	...
	public String readData(String path) {
		try {
			...
			return new String(this.zk.getData(path,new GetChildWatcher(),null)); //注意这个null，这里可以设置watcher
		} catch (KeeperException e) {
	...
	public List getChildList(String path) {
		try {
			...
			return zk.getChildren(path, new GetChildWatcher()) ;//这里可以设置watcher, zk.getChildren(path, new MyWatcher()) ;
		} catch (Exception e) {
	...
	public List readChildList(String path) {
		try {
			System.out.println("获取成功，path：" + path);
			return zk.getChildren(path, new ReadWatcher()) ;//这里可以设置watcher, zk.getChildren(path, new MyWatcher()) ;
		} catch (Exception e) {
			System.out.println("读取失败，发生Exception，path: " + path);
			e.printStackTrace();
			return null;
	...
	public List writeChildList(String path) {
		try {
			System.out.println("获取成功，path：" + path);
			return zk.getChildren(path, new WriteWatcher()) ;//这里可以设置watcher, zk.getChildren(path, new MyWatcher()) ;
		} catch (Exception e) {
			System.out.println("读取失败，发生Exception，path: " + path);
			e.printStackTrace();
			return null;
